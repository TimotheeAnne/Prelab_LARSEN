...
class HalfCheetahConfigModule:
    ...
    ENV_NAME = "PybulletHalfCheetahMuJoCoEnv-v0"
    TASK_HORIZON = 500
    NTRAIN_ITERS = 100
    ...
    def obs_cost_fn(obs):
        # return -obs[:, 0]
        if isinstance(obs, np.ndarray):
            return -obs[:, 0] + np.where(obs[:, 1] < -0.2, 1, 0)
        else:
            return -obs[:, 0] + tf.where(tf.less(obs[:, 1], -0.2), tf.ones(tf.shape(obs[:, 1])), tf.zeros(tf.shape(obs[:, 1])))
