...
class PexodQuad_env(gym.Env):
    ...
    def reset(self):
        ...
        if not self.__env_created:
            ...
            self.__hexapodId = self.p.loadURDF(self.boturdf,self.hexapodStartPos, self.hexapodStartOrientation, useFixedBase=0, flags=flags, physicsClientId=self.physicsClient) 
            ...
            self.p.changeDynamics(self.__planeId, linkIndex=-1, lateralFriction=self.lateral_friction, physicsClientId=self.physicsClient)
