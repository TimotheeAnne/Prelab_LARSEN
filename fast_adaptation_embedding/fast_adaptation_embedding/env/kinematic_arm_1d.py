...
class Arm_env(gym.Env):
    def __init__(self, goal=(100,180)):
        ...
        self.action_space = spaces.Box(low= -np.ones(1), high=np.ones(1))
        ...
        self.observation_space = spaces.Box(low= np.array([-1., -1., -1.]), high=np.array([1., 1., 1.])) #(x, y, sin_theta, cos_theta)1
    ...
    def step(self, act):
        ...
        if self.gui == True:
            self.render()
        return self.normalize(self.state)[[0,5,6]], -dist, done, {}
    ...
    def reset(self):
        ...
        return self.normalize(self.state)[[0,5,6]]
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        for k in range(100):
            if i%2 == 0:
                obs, r ,_ ,_ = system.step(np.ones(1))
            else:
               obs, r ,_ ,_ =  system.step(-np.ones(1))
            time.sleep(0.01)
        print(obs, r)
